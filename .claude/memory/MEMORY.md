...
- [CI-/Theme-Bridge Status](project_ci_theme_bridge.md) — Engine/Theme-Seite (1357b46), Spec (kdr14, 21e87a2), app.css-Pilot (AP-17, 6ea9704), QA-Kettenreview (17b–19, 8a4163a) committed; Font-Bridge umgesetzt (AP-17-FONT-CODE-A/B, uncommitted) — FwTheme.init() liest Fonts aus tokens.css, Canvas+HTML-UI token-gespeist, Spec-Status „implementiert (Mechanismus+Pilot)". `tools/ci-token-check.js` → `fwCiAudit()`: universelles Ist/Soll-Set-Audit (Farbe+Font, live aus tokens.css, .kg-card+Canvas), fand 2 echte CI-Bugs (TESTENV-1-FOLLOWUP-BTNFONT/-H3LEAK, 2026-07-11). Offen: App-Pool-Rollout, Rubikon-Nachmessung (DS-FOLLOWUP-07, fällig), Spacing/Ghost-Anbindung (T1), Harness-Versionierung (TESTENV-1, jetzt bis 1f GELB)
