...
description: "SEC-04 Ghost-Feed-Dateinamenvertrag (data-fw-data/data-fw-config als reine Dateinamen, zentraler AppDataResolver, JSONParser/FinanzwesirJsonData) + SEC-05 Theme-Bootstrapper-Registry (implementiert 2026-07-22) + Lehre: Testpflicht nur aus Registry ableiten, nie aus Ordner-Iteration"
metadata: 
  node_type: memory
  type: project
...
Zweite Entscheidung derselben Formalisierung: statischer Theme-Bootstrapper mit fester Registry/Slug-Whitelist (kein Code Injection, keine dynamischen Imports aus `data-*`-Werten) — **seit 2026-07-22 (SEC-05) implementiert:** `Theme/assets/js/apps/index.js` mit literaler `REGISTRY`, `prokrastinations-preis.js` aus `Apps/` dorthin verschoben, `initProkrastinationsPreis()`-Export statt globalem `bootstrap()`. P1-Korrektur direkt danach: `Object.hasOwn(REGISTRY, slug)` statt `REGISTRY[slug]`, da sonst geerbte Object-Prototype-Eigenschaften (`data-fw-app="toString"`) als Treffer durchgingen.

**Wichtige Folge-Lehre (Checker-Registry-Pflicht, 2026-07-22):** `tools/check-test-pages.py` `check_app_pflicht()` darf die Testpflicht **niemals** aus einer Ordner-Iteration über `Theme/assets/js/apps/*.js` ableiten — sobald ein statisch importiertes Hilfs-/Vertragsmodul (z. B. `prokrastinations-preis-stations-contract.js`) im selben Ordner liegt, entsteht ein Fehlalarm. Einzige Quelle der Wahrheit ist die literale `REGISTRY` in `index.js` (`extract_registry_slugs()`, fail-closed bei nicht eindeutigem Block). `TEST_PAGE_STANDARD.md` §12 entsprechend synchronisiert (v11–13).
