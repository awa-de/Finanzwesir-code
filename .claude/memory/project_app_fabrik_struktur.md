...
description: "docs/App-Fabrik Steuerungsverzeichnis; 22+3=25 App-Ordner; B1 APP_SPEC V2.5 operativ (Stationen-Zeitreise); Slice-6 ✅ 2026-06-16 (Altstand); B1-AP-01 bis AP-11 ✅ 2026-06-17; B1-AP-12 nächster Schritt (Stations-JSON validieren)"
metadata: 
  node_type: memory
  type: project
...
**B1-AP-11 ✅ 2026-06-17:** `loadStations()` implementiert in `app.js`. `config/stations.de.json` wird per `fetch` parallel zur CSV geladen (`Promise.all([loadData(), loadStations()])`). HTTP- und Parse-Fehler → `Error(d)`, kein stiller Fallback, keine Ersatzstationen. `stationsConfig` als 4. Parameter an `renderContent` übergeben (für AP-12 verfügbar). Nur `app.js` geändert, keine UI-Änderung. Nächster Schritt: **B1-AP-12 — Stations-JSON validieren.**
