...
Trigger: Albert sagt „fertig“, „finale Phase“, „abschließen“, „mach den Abschluss“ oder sinngemäß — oder Codex bietet den Skill proaktiv an, wenn ein AP erkennbar fertig ist.
...
Albert muss sich den Abschlussmodus nicht merken. Codex trägt die Prozessverantwortung.

Codex darf interaktiv fragen, wenn der Abschlussmodus nicht sicher ist. Das ist kein Fehler, sondern Drift-Schutz.
...
| `.Codex/learning/session-log.md` | lückenlose AP-Spur |
...
Codex denkt die Moduswahl intern durch. Sichtbar wird nur:
...
Codex verwendet diese Pfade direkt. Nicht im Root raten, außer der Pfad ist ausdrücklich Root.
...
| Session-Log | `.Codex/learning/session-log.md` |
| Memory-Verzeichnis | `.Codex/memory/` |
| Haiku-Scout | `.Codex/agents/abschluss-scout.md` |
| Haiku-Writer | `.Codex/agents/abschluss-writer.md` |
...
Codex prüft zuerst Kontext, AP-Status und eigene Edit-History. Wenn das nicht reicht, fragt Codex.
...
Codex kann ohne Rückfrage entscheiden, wenn der Fall eindeutig ist:
...
Wenn Codex nicht sicher ist, ob es Ketten-Minimalabschluss, Vollabschluss oder Miniabschluss ist, genau diese Frage stellen:
...
Wenn nur die technische Lage unklar ist, darf Codex gezielt prüfen:
...
1. `.Codex/learning/session-log.md`: AP-Eintrag.
...
1. `.Codex/learning/session-log.md`: knapper Eintrag zur Housekeeping-Aufgabe.
...
Bereiche: .Codex/skills/abschluss-ritual/SKILL.md, .Codex/agents/abschluss-writer.md
...
Eintrag in `.Codex/learning/session-log.md` mit Datum + AP-Titel.
...
- `[QUESTION]` Codex traf Annahme ohne Bestätigung
...
`.Codex/memory/` nur aktualisieren bei stabilen Projektfakten, Entscheidungen oder wiederverwendbarem Feedback.

Nach Änderungen in `.Codex/memory/`:
...
> **Wir sparen nicht, indem wir Genauigkeit weglassen. Wir sparen, indem Codex nicht rät, nicht unnötig liest, nicht labert und mechanische Arbeit nicht mit Sonnet erledigt.**
