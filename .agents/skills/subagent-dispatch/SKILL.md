...
In diesem Projekt: `CLAUDE_CODE_SUBAGENT_MODEL=haiku` ist in `.Codex/settings.local.json` gesetzt
und erzwingt Haiku für alle Subagenten zentral.
Agent-Dateien in `.Codex/agents/` setzen trotzdem `model: haiku` — damit die Absicht lokal sichtbar bleibt.
...
Vor jedem Subagent-Aufruf meldet Codex sichtbar:
...
Nach Rückkehr des Subagenten meldet Codex sichtbar:
...
Wenn Subagent-Zuarbeit laut Policy angezeigt ist, aber nicht genutzt wird, muss Codex sichtbar begründen:
