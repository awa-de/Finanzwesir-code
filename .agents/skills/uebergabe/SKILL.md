---
name: uebergabe
description: Erzeugt einen strukturierten Übergabeprompt für den nächsten Thread + schreibt automatisch einen Interim-Breadcrumb in session-log.md. Delta-Prinzip: nur AP-spezifisches Delta, kein Wiederholen was /start schon lädt.
---
...
- Codex erkennt proaktiv: Kontext ist weit gefüllt → Codex bietet an:
  „Soll ich einen Übergabeprompt erzeugen?"
- **MODUS M deklariert** → /uebergabe wird automatisch mitangeboten.
...
Nur das rein, was im Kopf von Codex steckt und nach dem Thread verloren wäre.
...
Codex beantwortet sich selbst (nicht ausgeben):
...
In `.Codex/learning/session-log.md` eintragen.

**Sprach-Prinzip (HRO: Signal, kein Blame):**
Einträge sind beschreibend, nicht evaluativ.
Richtig:   „[FRICTION] Gate-Schritt 7 übersprungen → manuell nachgeholt"
Falsch:    „[FRICTION] Fehler: Codex hat versagt"
...
„Gab es eine Überraschung?"               → j/n
    ...
    - [FRICTION] [Was passierte] → [Was korrigiert wurde]
...
Ein kalter Codex — neue Session, null Kontext — kann nach `/start` + Lesen dieses Prompts
...
| Gate-Regeln, Tabu-Zonen | In AGENTS.md, immer geladen |
| Projektstruktur, Pfade | In NAVIGATION.md, /start lädt das |
| Bereits committete Entscheidungen | In Spec-Dateien, Codex liest sie bei Bedarf |
