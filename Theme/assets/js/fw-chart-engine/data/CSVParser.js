...
export class CSVParser {
    ...
    async parse(url, options = {}) {
        ...
        try {
            ...
            return this._parseText(text, config);
        } catch (err) {
        ...
        for (let i = 1; i < lines.length; i++) {
            ...
            for (let j = 1; j < headers.length; j++) {
                ...
                if (valStr !== '') {
                    ...
                    if (!detectionDone) {
                        const detected = this._detectSuffix(valStr);
                        if (detected) {
                            unitKey = this.ALLOW_LIST[detected] || 'UNIT_UNKNOWN';
    ...
    _detectSuffix(str) {
        for (const s of Object.keys(this.ALLOW_LIST)) {
