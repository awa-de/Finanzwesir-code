...
Version: 1
Stand: 2026-07-22
Fokus-AP: Ghost-Feed-Resolver-Vertrag GHOST-05–09 ✅ + Theme-Bootstrapper/CSS-Reparatur-Kette ✅ (2026-07-22)
Nächster-Schritt: Albert prueft v3-Theme-ZIP in Ghost Local (Disclosure-Button + Regression der uebrigen Buttons + sieben CSS-Symptome); danach naechster Fokus nach Alberts Wahl (CSS-REPARATUR-KETTE ✅ 2026-07-22)
Blocker: keine
Letzter-Distill: 2026-07-20
Kassensturz-Datum: 2026-07-20
-->
<!-- HOOK-META-SESSION: Theme-Bootstrapper-Runtime-Grenze (SEC-05) + JSON-Offline-Validator + CSS-Architektur-Formalisierung (D-CSS-01–04) + CSS-Reparatur Option B + Checker-Registry-Fix + drei Theme-ZIPs (v1–v3), Kettenabschluss -->

# PROJECT STATUS — Finanzwesir 2.0

Stand: 2026-07-22 | Session: Theme-Bootstrapper/JSON-Validator/CSS-Reparatur-Kette (Kettenabschluss) | Geändert von: Claude
...
**Nächster Schritt:** Albert prüft das dritte Theme-ZIP (`finanzwesir-local-theme-prokrastinations-preis-v3.zip`) in Ghost Local: Disclosure-Button ohne native Kontur, kurze Regression der übrigen Buttons, sowie die sieben ursprünglichen CSS-Symptome + Reduced Motion + Viewports 375/768/1280px. Daneben weiterhin drei gleichrangige Fäden offen — Albert entscheidet situativ: **A) App-Duell-Runde:** 19 App-Fabrik-Apps sind bis zum Sonnet-Auftrag durchgezogen, Sonnet-Bau selbst steht app-weise noch aus (externe Sonnet-Fäden, Albert steuert). **B) Ghost.io-Prototyp:** SEO/GEO-Feldvertrag für `post.hbs` noch nicht geprüft/übertragen; T1s Produktions-Gate (Mengenvergleich je App, CDN-Ablösung, JANITOR-FALLBACK — P3-Punkt zu `tokens.css`-Inlining jetzt in BACKLOG T1 ergänzt); Albert überträgt `stations-de.json` nach `content/files/app-data/`. **C) Engine-DOM-AP:** Entscheidung zwischen Chart-Fallback-Option A (Tailwind-freie Testumgebungen begrenzt) und dem heutigen Übergangszustand C (Spiegel-CSS mit Paritätspflicht, D-CSS-01) bleibt offener, künftiger AP. Alle vier zuletzt bereits genannten Committed-Stände unverändert: App-Duell-Runde-Session (`fa86fec`), TMPL-1/T1/M1-Paket (`1ae7f5c`), die vorherige 09h–09k-Kette (`9986f8c`) sowie chart-engine CE-6/CE-6a (`2757580`).
