...
Version: 1
Stand: 2026-07-11
Fokus-AP: TESTENV-1f ✅ (GELB) — Abschlussgate der Testumgebungs-Kette: `TESTENV-1eB` (13 Engine-Zielseiten + 2 neue nach `tests/engine/`, 76 Fixtures nach `tests/fixtures/engine/`, Checker um 2 Ausnahmen erweitert, App-Testseite 32→31, GRÜN) abgeschlossen. `TESTENV-1f`: `.gitignore` bereinigt, automatisches Gate GRÜN (16 Testseiten, 0 Strukturfehler), Browser-Stichprobe (7 Seiten) formal offen (kein Browser-Werkzeug in dieser Umgebung) — GELB, wartet auf Alberts Rückmeldung. **Nachtrag (2026-07-11, gleicher Faden):** Auf Alberts Wunsch `tools/ci-token-check.js` um `fwCiAudit()` (primitives Ist/Soll-Set-Audit, erlaubte Farb-/Font-Menge live aus `tokens.css` diskoviert, kein Hardcode) + Canvas-Farbprüfung (`Chart.getChart`) erweitert, dreimal aus echten Testfunden präzisiert, Kopfkommentar um Erweiterungs-Anleitung für künftige LLMs ergänzt. Dabei 2 echte, bis dahin unbekannte CI-Bugs gefunden: `.fw-app__btn` rendert in Arial (Buttons erben `font-family` nicht automatisch) und `h3.fw-chart-title` leakt Hintergrund/Rahmen aus einem bei `TESTENV-1eB` unverändert übernommenen Legacy-`<style>`-Block (bare `h3`-Selektor, echte Migrations-Regression) — beide jetzt in `BACKLOG.md` (`TESTENV-1-FOLLOWUP-BTNFONT`/`-H3LEAK`), Reichweite der zweiten über weitere Engine-Seiten ungeklärt (Investigations-Auftrag fürs steuernde LLM). Kein Commit über die gesamte Kette (git log geprüft: HEAD weiterhin `09ec84c`, das war `TESTENV-1b–1eA`) — Commit-Freigabe liegt bei Albert.
Nächster-Schritt: Albert führt die Browser-Stichprobe aus `TESTENV-1f` §5 durch → Ergebnisdatei auf GRÜN ergänzen (kein neuer AP) → TESTENV-Kette committen. Parallel/danach: `TESTENV-1-FOLLOWUP-BTNFONT` (Ein-Zeilen-CSS-Fix, ready) und `TESTENV-1-FOLLOWUP-H3LEAK` (erst Reichweite über die anderen 12 Engine-Seiten mit `fwCiAudit()` klären, dann entscheiden). Weiterhin offen: App-Pool-Rollout Font-Bridge, Rubikon-Nachmessung (DS-FOLLOWUP-07, sachlich fällig). Ausdrücklich nicht ohne neuen Auftrag: Rubikon-Feinjustierung selbst, Tailwind, Design, andere Apps. Weitere offene BACKLOG-Kandidaten: AP-prokrast-17b-QA, DS-012/013, AP-prokrast-08-FOLLOWUP-A, TESTENV-1-FOLLOWUP-BORDER (Tailwind-gekoppelt), T1
Nebenabschluss: TESTENV-1eA ✅ (GELB) — Testumgebungs-Kette TESTENV-1b–1eA abgeschlossen: Vertrag (`TESTENV-1b`→`1bF`→`1bR`→`1bRF`, GRÜN, Standard = `docs/testing/TEST_PAGE_STANDARD.md`) → Infrastruktur gebaut (`TESTENV-1c`+`1cF`: Template, Shared CSS/JS, `tools/check-test-pages.py`, Launcher) → Altbestand klassifiziert (`TESTENV-1d`: 14 KEEP/1 MERGE/6 DELETE/2 OUTSIDE) → App-Testseite migriert (`TESTENV-1eA`: 32 Testfälle vollständig erhalten, Auftragszahl „34" war Zählfehler, mit Albert auf 32 korrigiert; Checker-Befunde 5→2, die 2 Restbefunde sind bewusste Negativtests der App [Szenario C, AF], mit Albert abgestimmt unverändert gelassen). Browser-Smoke von Albert real durchgeführt und **bestanden**; Albert hat `TESTENV-1eA` fachlich freigegeben. Alle Protokolle unter `docs/steering/patches/TESTENV-1{b,c,d,e}*_Ergebnis.md`, uncommitted (HEAD weiterhin `37ab2f9`). TESTENV-1a–TESTENV-1aFRF — Testumgebungs-Inventar (8 Protokolle: Anamnese, unabhängiger Review GELB, 4-teilige Korrekturkette, Korrektur-Review GRÜN, Freigabenachweis-Minifix). Kernfunde: 19 Dateien in `Theme/chart-tests/` (18 Legacy + 1 modern `AP-16-abnahme.html`), 91 Datendateien (81 `Theme/data/` + 7 App-Test-Fixtures + 3 B1-produktiv), 1 exakte HTML-Dublette (`index copy.html`/`index_balken_CI.html`), 1 gebrochene CSV-Referenz (`index_linen_2.html`→`bd_daily_90d.csv`), 1 Case-Sensitivity-Falle (`index_linien.html`→`scenario_6_decimals.csv` vs. reale `scenario_6_decimals.csV`), 25 App-Ordner (nur 1 mit `app.test.html`), `Theme/index.html` als tracked-aber-Fresh-Clone-brechende Sonderseite. Review fand und korrigierte: App-Ordner-Zählfehler (24→25), Fehlzuschreibung der Case-Sensitivity-Quelle (`index_alles test.html`→`index_linien.html`), Kategorienvermischung bei Orphan-Dateien, unbelegte Laufzeitbehauptung zu `Theme/index.html`. Kein Code-/Engine-/CSS-/`.gitignore`-Diff über die gesamte Kette. Kein Commit (git log geprüft: HEAD weiterhin `77f3229`) — Commit-Freigabe liegt bei Albert; AP-prokrast-17-FONT-SPEC-HEBUNG + Doku-Sync — Spec-Status KDR 14 P5 / TECH-SPEC §5.4 / CI-POOL §9 von „Ziel/Code offen" auf „implementiert (Mechanismus+Pilot)" gehoben (Label+Vorsatz+Statussatz einheitlich im Implementiert-Tempus, Albert-Rider gegen Kopf-aktualisiert-Body-alt-Drift eingearbeitet); repo-weiter Doku-Sync: `NAVIGATION.md` Z.243 (Rubikon-Folgeauftrag-Framing korrigiert), `BACKLOG.md` (AP-prokrast-17-FOLLOWUP-FONT als erledigt ins Archiv verschoben inkl. ehrlich benannter Abweichung von der ursprünglichen Rubikon-Kopplungsauflage — von Albert autorisiert entkoppelt; DS-FOLLOWUP-07 bleibt offen, Zusatz auf „jetzt sachlich fällig" aktualisiert, DS-012/013-Dep entfernt da Font bereits lokal via tokens.css lädt, nicht von Ghost-Prod-Anbindung abhängig; AP-prokrast-17b-QA-Querverweis auf DS-FOLLOWUP-07 umgehängt), `.claude/memory/project_ci_theme_bridge.md` (Font-Bridge-Umsetzung + Abweichung dokumentiert). AP-prokrast-17-FONT-CODE-B (HTML-UI-Fonts, Pfad B): `app.css` `--fw-font-base`→`--font-body` (Z.13-14 + Kopf-Kommentar), Headline-Entscheidung H-A (body-only, kein `--font-display`); `QA_TEST_CASES.md` TC-N07/TC-N09 auf neuen Soll umgeschrieben; B2–B7 (Engine-injizierte HTML-Flächen) durch Pfad A bereits token-gespeist, verify-only bestätigt (`FwRenderer`-Konstruktor ruft `this.theme.init()`, `_injectStyles`/`showError` ziehen aus `theme.fonts`). Albert-Browser-Verifikation (`app.test.html`): alle 5 Flächen (Fließtext, Screen-/Station-Headline, A11y-Tabelle, App-Meldung) computed `"Source Sans Pro"`, `document.fonts.check` ✅. Tool-Aufwertung `tools/ci-token-check.js`: `fwFontCheck()` neu (Font-Token- + Element-Prüfung), Härtungsfix (Match nur bei `In :root`, gegen Alberts Gegentest auf `Theme/chart-tests/index_alt.html` verifiziert — Legacy-Seite zeigt jetzt korrekt Match ❌ statt Fehlalarm). Nebenprodukt `TESTENV-1_harness-inventar_moderne-vs-legacy.md`: Merkmalsmatrix aller 20 Test-Harnesse (Kat. A `app.test.html`, Kat. B `AP-16-abnahme.html`, Kat. C 17× Legacy), Zielbild-Checkliste für spätere Vereinheitlichung. Kein Commit in dieser Kette (git log geprüft: HEAD weiterhin `dbe5007`) — Commit steht mit diesem Abschluss-Ritual an; AP-prokrast-17-FONT-CODE-A — Canvas-Font-Migration (Pfad A) + unabhängiges Review: `FwTheme.init()` liest `--font-body`/`--font-display` (Farb-Parität, KDR 14 Punkt 5), Constructor-Fallback erhalten; `getResponsiveFont(ctx, family)` reicht die Schrift aus der injizierten init()'ten Theme durch (`fontConfig`/`styleConfig.family` bei Line/Bar/Pie), Privatinstanz in FwLayoutRules nur noch Fallback; 3 tote Font-Parameter (Y-Achse, Tooltip title/body, X-Line) lebendig verdrahtet; Bar-Y auf flache Form migriert. 6 Engine-Dateien (`FwTheme.js`, `FwLayoutRules.js`, `FwSmartXAxis.js`, `FwSmartYAxis.js`, `FwSmartTooltips.js`, `BarChartStrategy.js`). Unabhängiges Claims-vs-Files-Review GRÜN (Opus, frische Instanz, nicht der Autor): 6/6 Prüfaufträge gegen die realen Dateien bestätigt, Null-Delta belegt (Token ≙ Constructor-Fallback, nur Quote-Stil), Scope exakt 6 Engine-Dateien, A6 (`FwChartTextPlugin.js:95`) unberührt, keine Fonts im fwContext, Spec-Status ehrlich „offen". Pfad-A-Paar (CANVAS-Write + REVIEW) gemeinsam archiviert (Write-AP-Ritual war im Vorfaden ausgefallen). Code + Review-Doku uncommitted (✅ 2026-07-10); AP-prokrast-17-FONT-ANAMNESE + AP-prokrast-17-FONT-SPEC-PARITÄT — Font-Einbindungs-Anamnese (26 Dateien, Python-deterministisch; Bruchstelle = Namens-Mismatch `--fw-font-base`/`--font-body`, Chart-Engine hat zweite ungebrückte Font-Quelle in `FwTheme.js`) + Fonts auf KDR-14-Parität zu Farben spezifiziert (`ARCHITECTURE STRATEGY PAPER VX.md` KDR 14 Punkt 5, `TECH-SPEC Theme-Integration Chart-Engine.md` §5.4, `CI-POOL-ROLLENKONTRAKT.md` §9), Status durchgängig „Code-Umsetzung offen". Kein Code-Diff. Committed a266cb2 (✅ 2026-07-10); AP-prokrast-17b/17c/18/19 — QA-Doku + unabhängiges Kettenreview + Abschluss-Korrektur: `QA_TEST_CASES.md` Gruppe N ergänzt (17b) + Übersichtstabelle mit Body synchronisiert (17c, Gruppe P nachgetragen); unabhängiges Claims-vs-Files-Review AP-15a–17c (18, Opus, GELB — 8/8 Substanz-Behauptungen GRÜN, Property-Zahl 19 vs. 20 aufgelöst, fünf Doku-Falschaussagen gefunden); Korrekturen umgesetzt (19, GRÜN — Kontrakt-Property-Zahl, TC-N05 neu formuliert + Korrektur-Nachtrag im 17b-Protokoll, `NAVIGATION.md`-Hex-Regel entschärft, `screen.css`-Kopf + Spec-Wortlaut synchronisiert). Kein Code-/Engine-Diff über die Vierer-Kette. AP-prokrast-16-FOLLOWUP-B damit erledigt (archiviert). Committed 8a4163a (✅ 2026-07-10); AP-prokrast-17 — Pilot-App-CSS-Farbmigration auf CI-Tokens (24 Farb-Referenzen `--fw-color-*`→`--color-*`, tokens.css-Link im Test-Harness, Font/Spacing bewusst ausgeklammert, Albert-Farbabnahme + Konsolen-ΔE-Check, QA-Tool `tools/ci-token-check.js`), committed 6ea9704 (✅ 2026-07-09); AP-prokrast-kdr14 — KDR-14-Spec-Wortlaut-Nachführung (`ARCHITECTURE STRATEGY PAPER VX.md`), committed 21e87a2 (✅ 2026-07-09); AP-prokrast-15a–16c — CI-/Theme-Bridge Umsetzung: Tailwind-CI-Pool-Inventar (15a, GRÜN), Rollen-/Benennungskontrakt CI-POOL-ROLLENKONTRAKT.md (15b, GELB nur Dateiname-Notiz), Farbleiter-Generierung + Abnahme-Board am Board abgenommen (15c, GELB→Pin 600 bestätigt), Theme-Migration tokens.css-SSoT + screen.css/FwTheme-Bridge (16, GRÜN), Bridge-Konsolidierung spec-konform + Kaskaden-Harness (16b, GRÜN — ChartEngine-Layer-Verstoß erkannt+revertiert, Injektion in LineChartStrategy Layer 3), Theme-Durchleitung an alle 3 Strategien per Composition Root (16c, GRÜN — Null-Delta 19/19 belegt + Albert-verifiziert), committed als 1357b46 (✅ 2026-07-09); AP-prokrast-14a–14c — CI-/Theme-Bridge Anamnese und Zielkontrakt (Inventar-Anamnese, unabhängiges Claims-vs-Files-Review, Zielkontrakt-/Architektur-Analyse mit Variante-C-Empfehlung, Rücklaufkapsel bewusst GELB), committed als d9ca9da (✅ 2026-07-07); AP-prokrast-12a–12c — RubikonSymbolMarkers Drehbuch-Errata-Sync (AP-prokrast-11-FOLLOWUP erledigt), unabhängig geprüft, Rücklauf an Masterfaden freigegeben (✅ 2026-07-07); AP-prokrast-11a–11d — Doku-/Spec-/QA-Sync (APP_SPEC.md, Drehbuch, QA_TEST_CASES.md) auf den abgenommenen Endstand aus AP-08/AP-10, LLM-lesbare Forensik-Rahmung eingeführt, unabhängig geprüft, Rücklauf an Masterfaden freigegeben, committed als 5365701 (✅ 2026-07-07); AP-prokrast-10a–10d — Screen-3-Reveal beim Übergang „Ergebnis ansehen" (Variante B++ Kontinuitäts-Reveal), unabhängig geprüft, Rücklauf an Masterfaden freigegeben, committed als 1033c9e (✅ 2026-07-07); AP-prokrast-09a–09d — Engine-Contract-Härtung nach AP-08 (chartSettled-Creation-Pfad geschlossen, No-op-Bootstrap bewusst offen zurückgegeben), teilweise abgenommen, unabhängig geprüft (✅ 2026-07-06); AP-prokrast-08a–08c — Card-to-Point-Motion Screen 2 gebaut, Architektur-Gate + harte Abschluss-QA GELB, committed als 18c87fb (✅ 2026-07-06); AP-prokrast-07a–07d — RubikonSymbolMarkers gebaut, CSS-feinjustiert, unabhängig geprüft, Rücklaufkapsel GRÜN, committed als ca45c94 (✅ 2026-07-06); AP-16/17/18 — Steuerungsblock-Rollout Statusabgleich + Batch C + unabhängiger Review (✅ 2026-07-01); AP-20/AP-20b/AP-21 — Sonderbatch D ohne plan-generator + unabhängiger Seed-Provenienz-Review (✅ 2026-07-01); AF-24/AP-22–25a — plan-generator-Seed-Sperre vollständig geklärt, Steuerungsblock-Rollout für alle 25 Apps (23 mechanisch + 2 dokumentierte Sonderfälle plan-generator/regulatorik-dashboard) abgeschlossen (✅ 2026-07-01); AP-prokrast-01–02e — neues amtliches Drehbuch prokrastinations-preis gegen Ist-Code/Architektur geprüft, Migrationsschnitt + unabhängige Abschluss-QA GRÜN, kein Code/Spec geändert (✅ 2026-07-02); AP-prokrast-03a–03e — Rubikon-Zukunftsraum architektonisch geklärt (03a/03b/03c) und als isoliertes `FwChartTextPlugin.js` gebaut (03d, additiver ChartEngine.js-Patch, Smart-Update-Testfall im Browser bestätigt), unabhängige Abschluss-QA GRÜN (03e), Commit empfohlen vor Screen-4-Integration (✅ 2026-07-02); AP-prokrast-03f–03i — Screen 4 (Rubikon-Reveal) vollständig gebaut: Integration zackig (03f, GELB), Root Cause auf Chart.js-4.5.0-Quellcode-Ebene geklärt (03g, GRÜN), Morph/C2 verworfen zugunsten stehendem Screen mit DOM-Overlay-Text im rechten Zukunftsfeld (03h/03h2, nach zwei Nachjustierungsrunden GRÜN, Albert bestätigt S/M/L+Konsole), unabhängige Abschluss-QA GRÜN, Screen 4 commitfähig (03i, ✅ 2026-07-02); AP-prokrast-04a–04c — APP_SPEC.md/Drehbuch auf Rubikon-Endstand synchronisiert (04a, GRÜN, §16.1a neu), unabhängige Abschluss-QA (04b, GRÜN, Fundstelle QA_TEST_CASES.md:557), Rücklaufkapsel mit zwei prominent eskalierten offenen Punkten (04c, GRÜN, ✅ 2026-07-03); AP-prokrast-05a–05e — QA_TEST_CASES.md auf Rubikon-Endstand synchronisiert (05a, GRÜN, TC-F01 neu + TC-F03/F04 neu), Abschluss-QA fand reale CTA-Fokus-Testlücke (05b, GELB, Rücklauf freigegeben), Light-Gate-Minifix schloss die Lücke (05c, GRÜN), Re-QA bestätigte die Schließung unabhängig (05d, GRÜN), Rücklaufkapsel mit Empfehlung an Masterfaden (05e, GRÜN, ✅ 2026-07-03); BACKLOG AP-27 damit erledigt; AP-prokrast-06a–06d — ✅/❓-Symbolik (Beat 2, BACKLOG AP-26) geklärt: Produktentscheidung GELB mit Empfehlung „streichen" (06a), danach Albert-Entscheidung außerhalb der Datei-Historie für Option A (reiner Canvas-Chart-Marker, kein DOM/A11y); APP_SPEC/Drehbuch/QA_TEST_CASES minimal synchronisiert (06b, GRÜN), unabhängig bestätigt (06c, GRÜN), Rücklaufkapsel mit Empfehlung AP-prokrast-07 (06d, GRÜN, ✅ 2026-07-04); BACKLOG AP-26 auf „entschieden, Bau offen" nachgezogen; AP-prokrast-07a — RubikonSymbolMarkers gebaut (GRÜN, 2026-07-06): `FwChartTextPlugin.js` additiv um `anchor:'lastPoint'` erweitert (bindet ✅/❓ an denselben Datenpunkt wie `FwVerticalLinePlugin`), `app.js` `renderScreen4Chart()` erweitert; AP-prokrast-07c — CSS-Feinjustierung nach Alberts Live-Test (2026-07-06): zwei Layout-Defekte am DOM-Textblock behoben (Kollision Screen S, fehlende Fluchtlinie zu „?"), neue M-Breakpoint-Stufe in `app.css` ergänzt, Diagnoseskript `tools/rubikon-symbol-markers-diagnose.js` neu; AP-prokrast-07b — Abschluss-QA Claims-vs-Files/TC-F05 (GELB, 2026-07-06): AP-07a vollständig bestätigt, M zum Prüfzeitpunkt offen, formaler DOM-/Accessibility-Tree-Check noch nicht dokumentiert, `CSS-KONVENTIONEN.md` als nicht einschlägig für App-eigene Stylesheets geklärt; AP-prokrast-07d — QA-Nachtrag + Rücklaufkapsel (GRÜN, ✅ 2026-07-06): Albert nahm M unter aktuellem Fallback-Font-Stand bewusst ab und führte den DOM-/Accessibility-Tree-Check durch (kein DOM-/A11y-/Live-Region-Treffer) — beide GELB-Gründe aufgelöst, TC-F05 für aktuellen Font-Stand bestanden, Commit aus AP-07-Sicht freigegeben; Folgeauftrag Rubikon-Neumessung nach CI-Font-Anbindung als BACKLOG DS-FOLLOWUP-07 registriert; BACKLOG AP-26 als erledigt archiviert
Blocker: keine
Letzter-Distill: 2026-07-06
Kassensturz-Datum: 2026-07-06
-->
<!-- HOOK-META-SESSION: TESTENV-1eB-1f-CI-AUDIT -->

# PROJECT STATUS — Finanzwesir 2.0

Stand: 2026-07-11 | Session: TESTENV-1eB-1f-CI-AUDIT | Geändert von: Claude
...
**Testumgebungs-Migration abgeschlossen, Abschlussgate GELB, CI-Audit-Tool ausgebaut ✅** (`TESTENV-1eB`/`TESTENV-1f` + Nachtrag, 2026-07-11): `TESTENV-1eB` hat die 13 fest vorgegebenen Engine-Testseiten + 2 neue (Security-Merge, Smart-Update-Auslagerung) nach `tests/engine/` migriert, 76 Fixtures nach `tests/fixtures/engine/` verschoben (SHA-256 geprüft), den Checker um 2 Ausnahmen erweitert (`data-fw-test-allow-missing-app-id`, `[data-fw-appchart]` unter `tests/engine/`) und die App-Testseite von 32 auf 31 Testfälle bereinigt (Szenario AF ausgelagert) — GRÜN. `TESTENV-1f` hat `.gitignore` bereinigt und das automatische Gate bestätigt (16 Testseiten, 0 Strukturfehler) — GELB, weil die Browser-Stichprobe (7 Seiten, `TESTENV-1f` §5) mangels Browser-Werkzeug in dieser Umgebung noch auf Alberts Rückmeldung wartet. **Nachtrag im selben Faden:** Auf Alberts Wunsch wurde `tools/ci-token-check.js` um ein primitives, universelles Ist/Soll-Set-Audit (`fwCiAudit()`) erweitert — die erlaubte Farb-/Font-Menge wird live aus `tokens.css` diskoviert (kein Hardcode, keine kuratierte Selektor-Liste), geprüft wird jeder tatsächlich benutzte Wert innerhalb der `.kg-card`-Bereiche einer beliebigen Testseite, zusätzlich über die Chart.js-Registry (`Chart.getChart`) auch Canvas-Farben (Serien, Tooltip, Plugins). Über drei echte Testrunden dreimal aus echten Funden präzisiert (Tooltip-`enabled:false`-Guard, `.kg-card`-Selbstausschluss, `accent-color` bei `range`/`checkbox`/`radio`). Kopfkommentar um eine „ERWEITERUNG"-Anleitung für künftige LLMs ergänzt (3 konkrete Erweiterungspunkte, Pflicht-Reihenfolge vor jeder Erweiterung — erst Quellcode belegen, dann erweitern). Dabei fand das Tool **2 echte, vorher unbekannte CI-Bugs**: `.fw-app__btn` rendert in Arial statt Source Sans Pro (Buttons erben `font-family` nicht automatisch von `.fw-app`), und `h3.fw-chart-title` (vom Chart-Engine zur Laufzeit erzeugt) leakt Hintergrund/Rahmen aus einem bei `TESTENV-1eB` unverändert übernommenen Legacy-`<style>`-Block mit bare `h3`-Selektor — eine echte, von der Migration selbst verursachte Regression, deren Reichweite über die anderen 12 migrierten Engine-Seiten noch ungeklärt ist. Beide als eigene `BACKLOG.md`-Zeilen verankert (`TESTENV-1-FOLLOWUP-BTNFONT`/`-H3LEAK`), dazu der bereits bekannte `.financial-chart-module`-`#ddd`-Rahmen (`TESTENV-1-FOLLOWUP-BORDER`, bewusst bis zur Tailwind-Arbeit zurückgestellt). Alle Protokolle unter `docs/steering/patches/TESTENV-1{eB,f}_*_Ergebnis.md` + 4 `PATCH-CI-AUDIT-TOOL-*`-Quittungen. Kein Layer-1–5-Produktionscode geändert. Kein Commit über die gesamte Kette (HEAD weiterhin `09ec84c`) — Commit-Freigabe liegt bei Albert, folgt nach der Browser-Stichprobe.

**Font-Bridge vollständig umgesetzt + Spec gehoben ✅** (AP-prokrast-17-FONT-CODE-A/B + SPEC-HEBUNG, 2026-07-10/11): Die Chart-Engine **und** die HTML-UI ziehen die Schrift jetzt — exakt wie die Farben seit AP-16 — aus `tokens.css`. `FwTheme.init()` liest zusätzlich `--font-body`/`--font-display` (KDR 14 Punkt 5); der Constructor-Hardcode bleibt Fallback für Test-HTMLs ohne Tokens. **Pfad A (Canvas, committed `dbe5007`):** `FwLayoutRules.getResponsiveFont(ctx, family)` nimmt die Schrift aus der injizierten, init()'ten Theme entgegen (X/Y-Achse, Tooltip); drei bis dahin tote Font-Übergabewerte lebendig verdrahtet. 6 Engine-Dateien, unabhängig reviewt (Opus, frische Instanz, GRÜN), Null-Delta belegt, A6 (`FwChartTextPlugin.js:95`) unberührt. **Pfad B (HTML-UI):** `Apps/prokrastinations-preis/app.css` `--fw-font-base` (nirgendwo definiert, Fallback `sans-serif`) → `--font-body`; App-Fließtext/-Headlines (H-A: body-only), A11y-Tabelle-im-Piloten und App-Meldungen ziehen automatisch mit. Engine-injizierte HTML-Flächen (Legende, Range-Buttons, BAN, Popover, Fehlermeldung) waren durch Pfad A bereits token-gespeist — verify-only bestätigt (`FwRenderer`-Konstruktor ruft `this.theme.init()`). **Albert-browserverifiziert** (`app.test.html`, `tools/ci-token-check.js`+neuer `fwFontCheck()`): alle 5 HTML-UI-Flächen computed `"Source Sans Pro"`, `document.fonts.check` ✅ — Schrift real geladen, kein stiller Fallback. **Spec-Status gehoben** (KDR 14 P5, TECH-SPEC §5.4, CI-POOL §9): „implementiert (Mechanismus + Pilot `prokrastinations-preis`)" — ausdrücklich **nicht** App-Pool; Rollout auf die übrigen Apps bleibt offen. Ergebnisprotokolle: `docs/steering/patches/AP-prokrast-17-FONT-CODE-A-CANVAS_Ergebnis.md`, `..._REVIEW_Ergebnis.md`, `..._CODE-B-HTMLUI_Ergebnis.md`, `..._SPEC-HEBUNG_Ergebnis.md`. Nebenprodukte: `tools/ci-token-check.js` (`fwFontCheck()` + Härtungsfix), `TESTENV-1_harness-inventar_moderne-vs-legacy.md` (Vorarbeit Test-Harness-Vereinheitlichung). **Abweichung von der ursprünglichen BACKLOG-Kopplungsauflage** (`AP-prokrast-17-FOLLOWUP-FONT`, jetzt archiviert): Migration und Rubikon-Nachmessung sollten gekoppelt laufen — real lief die Migration zuerst, die Nachmessung (DS-FOLLOWUP-07) bleibt offen, jetzt sachlich fällig; von Albert im CODE-B-Auftrag explizit autorisiert. Pfad B + Spec-Hebung + Nebenprodukte + Doku-Sync **uncommitted**, Commit steht mit diesem Ritual an. Nächster Haupt-AP (nach Commit): App-Pool-Rollout, Rubikon-Nachmessung, oder TESTENV-1.

**Font-Anamnese + Spec-Parität abgeschlossen ✅** (AP-prokrast-17-FONT-ANAMNESE + AP-prokrast-17-FONT-SPEC-PARITÄT, 2026-07-10): Breite Font-Einbindungs-Inventur (26 Dateien, Python-deterministisch) fand die reale Bruchstelle — kein fehlender Bridge, sondern ein Namens-Mismatch: `app.css` referenziert `--fw-font-base`, das SSoT-Token heißt `--font-body`/`--font-display` (`tokens.css`). Zusatzfund: die Chart-Engine hat eine zweite, nie CSS-gebrückte Font-Quelle (`FwTheme.js` hardcoded `this.fonts`, `init()` liest nachweislich nur Farben). `styles.css`/`stylesheet.css` beide tot bestätigt. Darauf aufbauend wurden Fonts im Spec auf KDR-14-Zielmechanismus-Parität zu Farben gehoben: `ARCHITECTURE STRATEGY PAPER VX.md` (KDR 14, neuer Punkt 5), `TECH-SPEC Theme-Integration Chart-Engine.md` (§5.4 umgeschrieben — Ist-Zustand lokal/DSGVO bereits real, veraltete Google-Fonts-CDN-Empfehlung gestrichen), `CI-POOL-ROLLENKONTRAKT.md` (§9 neuer Font-Bridge-Zielmechanismus-Absatz). Status damals durchgängig ehrlich: „Spec-Parität festgelegt — Code-Umsetzung offen" (mittlerweile durch AP-prokrast-17-FONT-CODE-A/B + SPEC-HEBUNG auf „implementiert" gehoben, s. oben). Farb-Staleness TECH-SPEC Z.286 bewusst nur geflaggt (Alberts Entscheidung). Kein Code-Diff. Committed `a266cb2`.

**QA-Doku + unabhängiges Kettenreview abgeschlossen ✅** (AP-prokrast-17b–19, 2026-07-10): `QA_TEST_CASES.md` Gruppe N (TC-N01–N09) für die AP-17-Farbmigration ergänzt (17b) + Übersichtstabelle mit Body synchronisiert (17c, vorbestehende Lücke Gruppe P nachgetragen). Unabhängiges Claims-vs-Files-Review der Gesamtkette AP-15a–17c (18, Opus, GELB): alle 8 load-bearing Substanz-Behauptungen bestätigt (GRÜN) — Null-Delta 19/19, Composition-Root/Constructor-Injection, Tabu-Zonen unberührt; Property-Zahl-Rätsel 20 (Kontrakt) vs. 19 (16c-QA) deterministisch aufgelöst (19 gelesene Properties in 20 Farb-Tokens, `linesDark` aus `textSec` abgeleitet); fünf Doku-Falschaussagen gefunden (u. a. `screen.css` fälschlich „Engine-intern" genannt, obwohl produktives Ghost-Stylesheet). Korrekturen umgesetzt (19, GRÜN): Kontrakt-Property-Zahl, TC-N05 neu formuliert + Korrektur-Nachtrag im 17b-Protokoll, `NAVIGATION.md`-Hex-Regel entschärft, `screen.css`-Kopf + Spec-Wortlaut synchronisiert. Kein Code-/Engine-Diff über die gesamte Vierer-Kette. AP-prokrast-16-FOLLOWUP-B damit erledigt. Committed `8a4163a`.

**Pilot-App auf CI-Tokens migriert ✅** (AP-prokrast-17, 2026-07-09): `Apps/prokrastinations-preis/app.css` — 24 CI-Farb-Referenzen `--fw-color-*` → Kontrakt-Tokens `--color-*` (8 Rollen, Fallbacks = tokens.css-FINAL); `tokens.css` als `<link>` in `app.test.html` (Dev-Harness, Ghost/Prod = T1). Primary wechselt bewusst Blau→Petrol (Kontrakt §1.1); muted/text/surface driften klein (ΔE ≤7). Font (`--fw-font-base`) + Spacing (`--fw-space-*`) bewusst NICHT migriert (Kontrakt §9/§7.6). Albert-Farbabnahme + Konsolen-ΔE-Check (5/5 Match). Neues QA-Tool `tools/ci-token-check.js`. Committed `6ea9704`.

**CI-/Theme-Bridge umgesetzt ✅** (AP-prokrast-15a–16c, 2026-07-09): `Theme/assets/css/tokens.css` als Single Source of Truth für Farben/Fonts/Schatten (volle Tailwind-kompatible Leitern 50–900 Petrol/Gelb/Purpur, Seeds gepinnt, am 15c-Board abgenommen); `screen.css` per `@import` angebunden, `:root`-Farbblock entfernt; `FwTheme`-Bridge liest tokens.css; Plugin-Farb-Hartcodierungen aufgelöst; **Theme-Durchleitung** an alle 3 Strategien per Composition Root (ChartEngine reicht das eine init()'te Theme durch — zweite Wahrheitsquelle geschlossen). Verbindliche Regeln: `docs/steering/design/CI-POOL-ROLLENKONTRAKT.md`. Null-Delta belegt (19/19 byte-identisch) + Albert am Live-Server-Harness verifiziert. CI-**Fonts** mittlerweile ebenfalls gebrückt (AP-prokrast-17-FONT-CODE-A/B, s. oben) — DS-012/013 (Ghost/Tailwind-Prod-Anbindung, T1) und DS-FOLLOWUP-08 (Card-to-Point-Timing) bleiben unabhängig davon offen. App-Pilot-Migration (AP-17 ✅), KDR-14-Spec-Nachführung (kdr14 ✅, committed 21e87a2) und Property-Zahl-Klärung (AP-prokrast-16-FOLLOWUP-B, aufgelöst durch AP-18/19) erledigt.
