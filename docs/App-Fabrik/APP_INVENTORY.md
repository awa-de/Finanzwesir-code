...
| `geburtsjahrlos` | B2 Geburtsjahrlos-Simulator | Master-App + Multi-Modul | eigenständig; rollierende-sparplaene als Erweiterungsmodul | Scenario Chart | Historische MSCI-/CPI-Daten (CSV + normiertes JSON) | Hoch — Fächer-Diagramm, Linien, Vergleich | Datenpipeline normieren, Chart-Engine-Integration, Verbindung zu rollierende-sparplaene (→ Fam-02) |
...
| `passiv-paradox` | G3 Passiv-Paradox | Master-App oder G2-Section | unklar — eigenständig oder eingebettet | Interactive Explainer | Textzustände/Annahmen (JSON) | Niedrig bis mittel | Entscheidung: eigene App oder Untersektion von G2 (→ Fam-01) |
...
| `rollierende-sparplaene` | B2 Geburtsjahrlos-Simulator | Erweiterungsmodul / Analysemodus | kein eigenständiger Funnel-Slot; Prototyp v2 vorhanden | Scenario Chart (Erweiterung) | MSCI NTR + CPI-Daten (CSV) | Hoch — rollierende Kohorten-Chart | Integration in B2, Datenpipeline normieren, Verbindungsmodell (→ Fam-02) |
...
| `weltkarte-etf-indizes` | C1 Diversifikations-Familie | Visuelles Lernmodul / Companion-App | eigenständig nutzbar; Prototyp v2 vorhanden | Explorer / Visual | Index-/Geo-Daten (JSON) | Hoch — interaktive Weltkarte (D3) | D3/TopoJSON lokal oder CDN (→ Arch-05), Verbindung zu C1 (→ Fam-03) |
