...
| AP-prokrast-16-FOLLOWUP | Design | KDR-14-Wortlaut-Nachführung: `docs/spec/ARCHITECTURE STRATEGY PAPER VX.md` KDR 14.2/14.3 auf die in AP-prokrast-16c umgesetzte Composition-Root-Theme-Durchleitung ergänzen (die Composition Root reicht das EINE init()'te FwTheme per Constructor Injection an die drei Strategien durch; Strategien haben Graceful Default). In 16c bewusst NICHT gemacht (keine Spec-Änderung im Umsetzungs-AP). Mini-AP, Diff-Anzeige, kein Prinzipienverlust | M | — | docs/steering/patches/AP-prokrast-16c_theme-durchleitung_Ergebnis.md |
