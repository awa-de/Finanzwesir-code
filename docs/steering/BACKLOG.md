...
Stand: 2026-07-11 | Session: TESTENV-1eB/1f + CI-Audit-Tool (Abschluss-Ritual) | Geändert von: Claude
...
| TESTENV-1-FOLLOWUP-BTNFONT | App | `.fw-app__btn` rendert in Arial statt Source Sans Pro — Buttons erben `font-family` nicht automatisch von `.fw-app`, `app.css:157` setzt keine eigene. Gefunden mit `tools/ci-token-check.js` → `fwCiAudit()`, 2026-07-11. Fix: `font-family: inherit;` in `.fw-app__btn` ergänzen, danach mit `fwCiAudit()` auf `app.test.html` bestätigen | H | — | docs/steering/patches/TESTENV-1f_browser-abschlussgate_Ergebnis.md §9 (vollständige Ursache + Diff-Vorschlag + Nachweis-Anleitung) |
| TESTENV-1-FOLLOWUP-H3LEAK | TESTENV/CSS | Regression aus `TESTENV-1eB`: bare Element-Selektor `h3 {...}` aus einem unverändert übernommenen Legacy-`<style>`-Block trifft jetzt zusätzlich das vom Chart-Engine zur Laufzeit erzeugte `h3.fw-chart-title` (`FwRenderer.js:124-126`) — Hintergrund/Rahmen leaken durch, Text-Farbe bleibt korrekt (Klassen-Selektor gewinnt dort). Bestätigt auf `tests/engine/tooltip.test.html`. **Offene Frage, die das steuernde LLM klären soll:** betrifft das auch die anderen 12 migrierten Engine-Seiten mit ähnlichem alten „farbige bare h2/h3-Überschriften"-Muster (`bar-ci`, `line-ci`, `pie-ci`, `tooltip-context`, `irregular-bar`, `irregular-line`, `cadence-density` u. a.)? Erst mit `fwCiAudit()` je Seite prüfen (Hintergrund/Rahmen-Zeilen), dann entscheiden: gezielt scopen (Klassen- statt Element-Selektor in den betroffenen Legacy-`<style>`-Blöcken) oder Einzelfall-Fix nur auf `tooltip.test.html` | M | — | tools/ci-token-check.js (`fwCiAudit()`, Fund 2026-07-11); tests/engine/tooltip.test.html |
...
| TESTENV-1 | App-Fabrik/Test | **Initiative: Konsolidierte Testumgebung.** Phasen 1–4 (Inventar, Vertrag, Infrastruktur, Klassifikation) GRÜN abgeschlossen. Phase 5 (Migration): `TESTENV-1eA` (App-Testseite, GELB) + `TESTENV-1eB` (13 Engine-Zielseiten + 2 neu, 76 Fixtures, Checker-Erweiterungen, GRÜN) abgeschlossen. `TESTENV-1f` (Abschlussgate): automatisches Gate GRÜN (16 Testseiten, 0 Strukturfehler, `.gitignore` bereinigt), Browser-Stichprobe formal offen (kein Browser-Werkzeug in dieser Umgebung) — GELB. Nachtrag: neues CI-Audit-Tool (`tools/ci-token-check.js` → `fwCiAudit()`) fand dabei 2 echte Bugs, siehe eigene BACKLOG-Zeilen `TESTENV-1-FOLLOWUP-BTNFONT`/`-H3LEAK`/`-BORDER`. Alle Protokolle unter `docs/steering/patches/TESTENV-1*_Ergebnis.md` | M | Browser-Stichprobe (Albert, TESTENV-1f §5) | Kontext: `docs/steering/patches/TESTENV-1f_browser-abschlussgate_Ergebnis.md` (Nächster Schritt bei GRÜN: TESTENV-Kette committen) |
