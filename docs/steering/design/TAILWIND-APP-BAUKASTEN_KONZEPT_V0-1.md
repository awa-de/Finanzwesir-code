...
| Legend-Pill | `rounded-full` | `shadow-soft`, hover `shadow-hover` | funktional `border border-border`, hover `border-primary` | `px-3 py-1`, Gruppe `gap-2` |
...
- Content: `mt-2 pl-3`
- Zwischenwertgruppe (wenn der Inhalt Kennzahlen enthält): `dl` mit `m-0 grid w-full grid-cols-2 gap-x-4 gap-y-1 sm:w-fit`; **jede Kennzahl ist ein eigenes `div` im Grid** und enthält `dt` (`text-xs text-text-muted`) über `dd` (`m-0 text-base font-semibold text-text`). Das Grid ordnet Kennzahl-Blöcke nebeneinander an, nie vier einzelne `dt`/`dd`-Zellen.
...
**Rezepte:**
- Wrapper: `flex flex-col gap-3` (im App-Kontext ohne eigene Fläche — die App liefert den Grund; Standalone-Nutzung darf `rounded-xl bg-bg p-4` ergänzen). Auf dauerhaften Tailwind-freien Engine-Testseiten stellt ein gleichwertiges tokenbasiertes Fallback (`display:flex; flex-direction:column; gap:12px`) denselben Rhythmus zwischen Titel, BAN, Toolbar, Legende und Canvas her — reine Parität, keine zweite Designquelle.
- Titel: `m-0 text-lg font-bold text-primary`
- Toolbar: `flex flex-wrap items-center gap-2` (+ Control-Group-Rezepte 6.5). Bei zwei Control-Gruppen (z. B. Zeitspanne und Ansichtsauswahl) steht die zweite Gruppe auf M/L rechts (`ml-auto`); auf Zone S (Engine-Container ≤450px) bricht sie bei Bedarf linksbündig um, ohne künstliches Toolbar-Padding oder zentrierte zweite Zeile.
- BAN: Container `inline-block min-w-48 self-start rounded-lg bg-bg-faint px-4 py-3` mit `aria-live="polite"` (spiegelt die Bestandsoptik: getönte, gerundete, kompakte, startausgerichtete Inline-Box — nicht breitenfüllend; Ton-in-flächenlosem-Chrome ist Card-analog erlaubt); Hauptwert `text-xl font-bold text-text` (Zone >450px: `text-2xl` via Engine-Container-Query); Subzeile `text-sm text-text-sec`; Hinweis `text-sm text-text-sec`
- Canvas-Container: `relative w-full` (Höhensteuerung bleibt Engine/Chart.js — kein Tailwind-Eingriff)
- Legende: Gruppe `flex flex-wrap gap-2`; Pill `inline-flex cursor-pointer select-none items-center gap-2 rounded-full bg-bg px-3 py-1 text-sm shadow-soft transition-shadow motion-reduce:transition-none hover:shadow-hover focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-petrol-500 border border-border text-text transition-colors hover:border-primary hover:bg-bg-faint hover:text-primary`; Pill (Serie ausgeblendet, vollständiger Alternativ-String): zusätzlich `opacity-40 grayscale`; Dot `h-2.5 w-2.5 rounded-full` mit **Inline-Style-Farbe aus der Engine-Serienfarbe** (dokumentierte Nicht-Tailwind-Ausnahme, 2.2 Punkt 5); Pills werden `<button type="button" aria-pressed>` (schließt F-09 konzeptionell). Die feine Kontur ist die in D-02 dokumentierte funktionale Ausnahme für bedienbare Serien-Toggles, nicht ein neuer dekorativer Standard.
  **App-Fabrik-Verbindlichkeit:** Jede App, die dieses Primitive nutzt, verwendet dieses Rezept und dieselben Ruhe-, Hover-, Fokus- und Toggle-Zustände. Eine Abweichung ist nur bei belegter abweichender Funktion zulässig, nicht als optische App-Variante.
  **Basis/Bedeutung-Trennung (DOC-04, Alberts Produktentscheidung, 2026-07-15):**
  1. **Ein gemeinsames visuelles Basis-Primitive:** Alle interaktiven Legend-Pills verwenden dieselben bestehenden Rezepte für Ruhe, Hover und Fokus — Ruhe: `bg-bg`, `border-border`, `text-text`, `shadow-soft`; Hover: `border-primary`, `bg-bg-faint`, `text-primary`, `shadow-hover`, kein Lift; Fokus: `focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-petrol-500`. Keine neuen Farb-, Schatten-, Spacing- oder Radiuswerte, keine abweichende lokale Hover-Farbfläche.
  2. **Fachliche Bedeutung ist getrennt von der Basisoptik:** Datenreihen-Sichtbarkeit — ausgeblendet = `opacity-40 grayscale`. Segment-Dämpfung — nur der bestätigte Pie-aktive/ghost-Zustand bleibt eigenständig; das Segment bleibt sichtbar, die Canvas-Dämpfung bleibt fachlich unverändert. Der DOM-Ghost überlagert die gemeinsame Basis gezielt mit `background: --color-grid` (Bridge-Token, keine Utility), transparenter Border, keinem Schatten und `opacity: .6`; sein Text ist `--color-text-disabled` und durchgestrichen, sein Dot `grayscale(100%)` mit `opacity: .5`. Er hat keinen aktiven Hover/Lift und wird ausschließlich durch den realen Pie-Zustand gesetzt.
  3. Die Strategy benennt nur die benötigte Legendenbedeutung; sie besitzt keine Klassen, CSS-Werte oder Hover-Entscheidungen. Der Renderer liefert die gemeinsame Basisoptik und spiegelt nur den jeweiligen fachlichen Zustand.
  4. Eine abweichende Fachbedeutung rechtfertigt keine abweichende Ruhe-/Hover-/Fokusoptik. Sie rechtfertigt ausschließlich den belegten Zustandsunterschied.
...
**Responsive:** Engine-Container-Query, 2 Zonen (D-10): Zone S stapelt Toolbar unter den Titel und lässt Pills umbrechen — beides leistet `flex-wrap` bereits, die Engine behält ihre `@container`-Steuerung für Sonderfälle. Bei Linie bleibt die Toolbar-Zeile selbst auf Zone S (≤450px) erhalten; nur eine zweite Control-Gruppe bricht bei Bedarf natürlich linksbündig um. Legend-Pills brechen in jeder Zone natürlich über `flex-wrap`, ohne künstliche Spalten oder Zusatzpadding.
...
| Legende (vor dem Canvas) | Pills je Serie, `aria-pressed`-Toggle | Pills je Serie | Gemeinsame Legend-Pill-Basisoptik wie Line/Bar (DOC-04); Bedeutung = Segment-Dämpfung umschalten (entschieden, DOC-03). Nur der aktiv/ghost-Zustand und die Canvas-Wirkung bleiben Pie-spezifisch — Kategorienliste als abweichende Bedeutung und Drill-down-Popover bleiben offen |
