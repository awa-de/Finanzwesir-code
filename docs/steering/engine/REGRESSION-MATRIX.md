...
| REG-DOM-002 | DOM-Chrome | Donut/Pie-Chart-Chrome — Segment-Dämpfung als eigenständiges Legend-Pill-Primitiv (`aria-pressed`, gemeinsame Basisoptik mit Line/Bar seit CE-5d), gemeinsamer `focus-visible`-Fallback für Segmented-Option/Legend-Pill/Pie-Segment-Dämpfung (AP-chart-engine-01 CE-5/CE-5a/CE-5b/CE-5c/DOC-04/CE-5d) | tests/engine/pie-ci.test.html + tests/engine/line-ci.test.html + tests/engine/bar-ci.test.html (alle Tailwind-frei) | Pie-Legendeneinträge sind echte `<button type="button" aria-pressed>`; Klick dämpft genau ein Segment (aktiv↔ghost), Segment bleibt sichtbar, Daten/Canvas-Mechanik unverändert, kein Drill-down über die Legende; Ruhe/Hover/Fokus der aktiven Pie-Pill identisch zu Line/Bar (kein petrol-getönter Hover, kein Lift); Fokusring überall derselbe Petrol-500-Ring ohne Offset (`c.petrol80`); Ghost-Zustand (`hidden-dataset`) bleibt bei Hover unverändert gedämpft; Ranking-Bar zeigt weiterhin keine Legende | FwRenderer.js (`_renderLegend()` isPie-Zweig, `_setPieSegmentDampingState()`, `_injectStyles()` Chrome-Fallback), ChartEngine.js (`_bindLegendEvents()` Pie-Zweig), PieChartStrategy.js (`handleLegendClick()`, unverändert) | Teilweise — `tools/engine-dom-check.js` Segment-Dämpfungs-Struktur-Check (Button/`aria-pressed`/Fokussierbarkeit/Initialzustand, 9/9 PASS real bestätigt) + `tools/pie-segment-damping-interaction-check.js` (Klick-Rundreise/andere Segmente unverändert/kein Drill-down/Wiederherstellung, 51/51 PASS real bestätigt); Hover-/Fokus-Sichtprüfung bleibt manuell |
