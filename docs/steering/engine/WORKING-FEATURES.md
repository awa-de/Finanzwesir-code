...
- Engine-DOM-Chrome auf Tailwind-Baukasten migriert (AP-chart-engine-01, CE-2b bis CE-5d, 2026-07-14/15; CE-1–CE-4c/DOC-01/DOC-02/DOC-02a committed, **CE-5-Preflight bis CE-5d noch nicht committed**): Gemeinsames DOM-Fundament (Wrapper/Canvas-Container/A11y-Tabelle/Loading/Error) trägt Baukasten-Literalrezepte (§6.9–6.11) + einen schmalen tokenbasierten CSS-Fallback für die dauerhaft Tailwind-freien Engine-Testseiten. Für **Linie und regulären Mehrserien-Bar**: Titel/Toolbar/Segmented-Controls/Legende (echte `<button aria-pressed>`-Pills) als **gemeinsame `FW_CHROME_*`-Konstanten** in `FwRenderer.js` (seit CE-4c, nicht mehr als duplizierte `FW_LINE_*`/`FW_BAR_*`-Paare), mit gemeinsamem CSS-Fallback unter `.fw-chart-chrome`; typspezifische Wrapper-Marker (`.fw-chart-wrapper--line`/`--bar`) zusätzlich im DOM. BAN bleibt line-exklusiv. Ranking-Bar zeigt weiterhin keine Legende. **Donut/Pie (seit CE-5/CE-5d):** Legende bedeutet „Segment-Dämpfung umschalten“ (DOC-03, vierter Legend-Bedarf neben Line/Bar-Dataset-Sichtbarkeit) — echte `<button type="button" aria-pressed>`-Pills, Zustand aus `PieChartStrategy._status` gelesen (keine zweite Quelle, kein unabhängiges `classList.toggle()`), Klick dämpft/entdämpft genau ein Segment (Canvas + DOM synchron), kein Drill-down über die Legende (bleibt separater Canvas-Klickpfad für „Weitere …“). Aktive Pie-Pill teilt seit CE-5d dieselbe Ruhe-/Hover-/Fokus-Basisoptik wie Line/Bar (`FW_CHROME_LEGEND_PILL_CLASS`/`FW_CHROME_LEGEND_DOT_CLASS` direkt referenziert, kein petrol-getönter Hover/Lift mehr — DOC-04-Produktentscheidung); Ghost-Zustand bleibt eigenständig (`hidden-dataset`, unverändert). Gemeinsamer `focus-visible`-Fallback (Petrol-500-Ring ohne Offset, `c.petrol80`) für Segmented-Option/Legend-Pill/Pie-Segment-Dämpfung seit CE-5c. Diagnosewerkzeuge: `tools/engine-dom-check.js` (A11y-Verbergung, Struktur-Anker, Chrome-Kern-Block seit CE-4c, Segment-Dämpfungs-Struktur-Block seit CE-5 — alles read-only) sowie `tools/pie-segment-damping-interaction-check.js` (NEU, bewusst nicht read-only: Klick-Rundreise dämpfen/wiederherstellen). Ergebnisprotokolle: `docs/steering/patches/AP-chart-engine-01_CE-2b_*` bis `..._CE-5d_*`, `..._DOC-02_*` bis `..._DOC-04a_*`.
