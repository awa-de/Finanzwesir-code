...
- Engine-DOM-Chrome auf Tailwind-Baukasten migriert (AP-chart-engine-01, CE-2b + CE-3/3a/3b + CE-4/CE-4c, 2026-07-14/15, **noch nicht committed**): Gemeinsames DOM-Fundament (Wrapper/Canvas-Container/A11y-Tabelle/Loading/Error) trägt jetzt Baukasten-Literalrezepte (§6.9–6.11) + einen schmalen tokenbasierten CSS-Fallback für die dauerhaft Tailwind-freien Engine-Testseiten (`.fw-chart-a11y-fallback`, `.fw-chart-error` u. a.). Für **Linie und regulären Mehrserien-Bar** zusätzlich: Titel/Toolbar/Segmented-Controls (Zeitspanne + Ansicht, echte `<button role="group" aria-pressed>`)/Legende (echte `<button aria-pressed>`-Pills) auf Baukasten-Literale (§6.5/§6.11) migriert — seit CE-4c als **gemeinsame `FW_CHROME_*`-Konstanten** in `FwRenderer.js` geführt (nicht mehr als duplizierte `FW_LINE_*`/`FW_BAR_*`-Paare), mit gemeinsamem CSS-Fallback unter dem Anker `.fw-chart-chrome`; typspezifische Wrapper-Marker (`.fw-chart-wrapper--line`/`--bar`) bleiben zusätzlich im DOM. BAN bleibt line-exklusiv (`FW_LINE_BAN_*`, unverändert). Ranking-Bar zeigt weiterhin keine Legende (unverändert, `interactiveFilters` hartcodiert `false`). Donut/Pie unverändert (kein Marker/Anker, eigener, späterer Slice CE-5). Diagnosewerkzeug `tools/engine-dom-check.js` prüft A11y-Verbergung + Struktur-Anker über beide Datenpfade, seit CE-4c zusätzlich einen Chrome-Kern-Block (Typmarker+Anker, `<button aria-pressed>`-Controls/-Pills, genau ein aktiver Button pro Gruppe). Ergebnisprotokolle: `docs/steering/patches/AP-chart-engine-01_CE-2b_*` bis `..._CE-4c_*`.
