...
- Engine-DOM-Chrome auf Tailwind-Baukasten migriert (AP-chart-engine-01, CE-2b bis CE-6a, 2026-07-14/15; CE-1–CE-4c/DOC-01/DOC-02/DOC-02a/CE-5-Preflight/DOC-03/CE-5/CE-5a-c/DOC-04/DOC-04a/CE-5d committed (`20bb90c`…`93c884f`), **nur CE-6/CE-6a noch nicht committed**) — **Kette CE-1 bis CE-6a vollständig abgeschlossen und browserabgenommen** (CE-6: read-only Cross-Type-Nachweis über alle drei Charttypen statisch GELB; CE-6a: Alberts reale S/M/L-Rückmeldung „Alles ok." als manuelle Abnahme quittiert → GRÜN): Gemeinsames DOM-Fundament (Wrapper/Canvas-Container/A11y-Tabelle/Loading/Error) trägt Baukasten-Literalrezepte (§6.9–6.11) + einen schmalen tokenbasierten CSS-Fallback für die dauerhaft Tailwind-freien Engine-Testseiten. Für **Linie und regulären Mehrserien-Bar**: Titel/Toolbar/Segmented-Controls/Legende (echte `<button aria-pressed>`-Pills) als **gemeinsame `FW_CHROME_*`-Konstanten** in `FwRenderer.js` (seit CE-4c, nicht mehr als duplizierte `FW_LINE_*`/`FW_BAR_*`-Paare), mit gemeinsamem CSS-Fallback unter `.fw-chart-chrome`; typspezifische Wrapper-Marker (`.fw-chart-wrapper--line`/`--bar`) zusätzlich im DOM. BAN bleibt line-exklusiv. Ranking-Bar zeigt weiterhin keine Legende (`interactiveFilters:false` → `_renderLegend()` liefert `null`, CE-6 strukturell bestätigt). **Donut/Pie (seit CE-5/CE-5d):** Legende bedeutet „Segment-Dämpfung umschalten“ (DOC-03, vierter Legend-Bedarf neben Line/Bar-Dataset-Sichtbarkeit) — echte `<button type="button" aria-pressed>`-Pills, Zustand aus `PieChartStrategy._status` gelesen (keine zweite Quelle, kein unabhängiges `classList.toggle()`), Klick dämpft/entdämpft genau ein Segment (Canvas + DOM synchron), kein Drill-down über die Legende (bleibt separater Canvas-Klickpfad für „Weitere …“). Aktive Pie-Pill teilt seit CE-5d dieselbe Ruhe-/Hover-/Fokus-Basisoptik wie Line/Bar (`FW_CHROME_LEGEND_PILL_CLASS`/`FW_CHROME_LEGEND_DOT_CLASS` direkt referenziert, kein petrol-getönter Hover/Lift mehr — DOC-04-Produktentscheidung); Ghost-Zustand bleibt eigenständig (`hidden-dataset`, per `:not(.hidden-dataset)` explizit vor dem gemeinsamen Hover-Selektor geschützt, CE-6 bestätigt). Gemeinsamer `focus-visible`-Fallback (Petrol-500-Ring ohne Offset, `c.petrol80`) für Segmented-Option/Legend-Pill/Pie-Segment-Dämpfung seit CE-5c. Diagnosewerkzeuge: `tools/engine-dom-check.js` (A11y-Verbergung, Struktur-Anker, Chrome-Kern-Block seit CE-4c, Segment-Dämpfungs-Struktur-Block seit CE-5 — alles read-only) sowie `tools/pie-segment-damping-interaction-check.js` (NEU, bewusst nicht read-only: Klick-Rundreise dämpfen/wiederherstellen). Vier bewusst offene, nicht blockierende Restpunkte (KDR-15-Chrome-Auftrag technisch noch nicht durchgeleitet, F-06 Pie-Popover unbewertet, Pie-Wrapper/-Titel-Vollmigration zurückgestellt, Donut-Kategorienliste offen) siehe CE-6-Ergebnisprotokoll. Ergebnisprotokolle: `docs/steering/patches/AP-chart-engine-01_CE-2b_*` bis `..._CE-6_cross-type-abschluss-und-engine-dom-uebergabe_Ergebnis.md`, `..._DOC-02_*` bis `..._DOC-04a_*`.
