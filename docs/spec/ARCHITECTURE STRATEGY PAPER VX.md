...
### KDR 15: Semantischer Chrome-Auftrag (Strategy-zu-Renderer-Grenze) — Zielvertrag, technische Durchleitung noch nicht implementiert
* **Kontext:** Die Bar/Line-Chrome-Migration (CE-3/CE-3b/CE-4/CE-4c) hat die Renderer-seitigen Chrome-Rezepte bereits zentralisiert (`FW_CHROME_*`-Konstanten, gemeinsam genutzt von Layer 3 `LineChartStrategy`/`BarChartStrategy` über den Renderer), aber noch nicht formal von der Strategie-Ebene entkoppelt. Vor CE-5 (Donut/Pie-Chart-Chrome) wird die dahinterliegende Layer-Grenze verbindlich dokumentiert, damit kein künftiger Charttyp wieder Stylewissen in die Strategie zieht oder eine zweite Chrome-Fabrik entstehen lässt (vgl. die CE-4-Lehre: elf duplizierte `FW_BAR_*`-Konstanten mussten in CE-4c auf gemeinsame `FW_CHROME_*`-Rezepte zurückgeführt werden).
* **Entscheidung (Zielvertrag — dieser KDR-Eintrag ändert keinen Ist-Code):**
    1. Strategien (Layer 3, `LineChartStrategy`/`BarChartStrategy`/`PieChartStrategy`) liefern fachlich-semantische Render-Absicht; sie erzeugen keine DOM-/Tailwind-/CSS-Rezepte.
    2. Der Renderer (Layer 5, `FwRenderer`) besitzt die vollständigen statischen Chrome-Rezepte, die HTML-Reihenfolge, die A11y-Semantik, die Interaktionsoberflächen und die tokenbasierten Fallbacks für Tailwind-freie Testseiten.
    ...
    4. Der semantische Chrome-Auftrag ist request-scoped und unveränderlich wie der Rucksack (KDR 9) — er folgt demselben Context Object Pattern, enthält aber keine CSS-/Tailwind-/DOM-Werte. Seine technische Durchleitung von der Strategie zum Renderer wird in einem späteren Code-AP umgesetzt; dieser Eintrag führt ausdrücklich keine neue produktive API und keinen Pseudocode als vermeintlich umgesetzten Ist-Stand ein.
    5. Chart.js-Canvas, Plugins und fachliche Charttyp-Physik (Achsen, Datasets, Tooltips, Center-Text) bleiben auf ihren bestehenden Layern unberührt von diesem Vertrag.
* **Abgrenzung zu KDR 9:** KDR 9 (Der Rucksack) bleibt unverändert gültig für Darstellungssemantik (Einheit, Rhythmus, Zeit-/Ansichtsmodus, Datenbereich, Anker). Der Chrome-Auftrag ist ein eigenständiger, strukturell verwandter, aber inhaltlich getrennter Vertrag für DOM-Chrome-Bedarfe (Headline/Range-Control/View-Control/Legende/Caption, siehe `docs/steering/design/TAILWIND-APP-BAUKASTEN_KONZEPT_V0-1.md` §6.11). Er erweitert `fwContext` nicht rückwirkend und wird nicht mit ihm vermischt, solange keine technische Durchleitung implementiert ist.
* **Status:** Konzeptionell verankert (2026-07-15, DOC-02). Die Rezeptzentralisierung für Line/Bar (`FW_CHROME_*`) ist implementiert (CE-4c). Die semantische technische Durchleitung (ein expliziter Chrome-Auftrag von der Strategie zum Renderer) ist noch nicht implementiert.
* **Begründung:** CE-5 (Donut/Pie) und künftige Charttypen brauchen eine verbindliche Grenze vor der Umsetzung, nicht danach — sonst wiederholt sich das CE-4-Muster (Duplikat-Rezepte statt Wiederverwendung).
